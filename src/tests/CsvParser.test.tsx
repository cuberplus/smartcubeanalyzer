...
describe('CsvParser', () => {
    test('parses Cubeast CSV into solves', () => {
        ...
        // Cubeast export provides solving_time in ms; Solve.time should prefer that.
        expect(solve.time).toBeCloseTo(25.755, 3); // 25755 ms -> 25.755 s
    });

    test('Cubeast Solve.time falls back to time - pickup_time - putdown_time when solving_time is missing', () => {
        const header = `id,date,dnf,time,solving_method,solution_rotation,pickup_time,putdown_time,solving_time`;
        // time=20000, pickup_time=500, putdown_time=1500, solving_time empty -> expected (20000-500-1500)=18000 ms => 18s
        const row = `cubeast-row,2026-01-05 15:09:52,false,20000,CFOP,UF,500,1500,`;
        const csv = `${header}\n${row}\n`;
        const solves: Solve[] = parseCsv(csv, ',');
        expect(solves.length).toBe(1);
        const solve = solves[0];
        expect(solve.source).toBe('cubeast');
        expect(solve.time).toBeCloseTo(18.0, 3);
    });
    ...
    test('Acubemy analysis_type Roux sets method and initializes Roux steps (empty)', () => {
        const csv = `solve_id,date,total_time,analysis_type,cross_face
1,2026-01-08T10:32:50.222Z,10000,Roux,U`;
        const solves: Solve[] = parseCsv(csv, ',');
        expect(solves.length).toBe(1);
        const solve = solves[0];
        expect(solve.source).toBe('acubemy');
        expect(solve.method).toBe(MethodName.Roux);
        expect(solve.steps[0].name).toBe(StepName.LEFTBLOCK);
        expect(solve.steps[1].name).toBe(StepName.RIGHTBLOCK);
        expect(solve.steps[2].name).toBe(StepName.CMLL);
        expect(solve.steps[3].name).toBe(StepName.LSE);
        expect(solve.steps[0].time).toBe(0);
        expect(solve.steps[1].turns).toBe(0);
    });

    test('Acubemy analysis_type ZZ sets method and initializes ZZ steps (empty)', () => {
        const csv = `solve_id,date,total_time,analysis_type,cross_face
1,2026-01-08T10:32:50.222Z,10000,ZZ,U`;
        const solves: Solve[] = parseCsv(csv, ',');
        expect(solves.length).toBe(1);
        const solve = solves[0];
        expect(solve.source).toBe('acubemy');
        expect(solve.method).toBe(MethodName.ZZ);
        expect(solve.steps[0].name).toBe(StepName.EOLINE);
        expect(solve.steps[1].name).toBe(StepName.F2L);
        expect(solve.steps[2].name).toBe(StepName.ZBLL);
        expect(solve.steps[0].time).toBe(0);
        expect(solve.steps[1].turns).toBe(0);
    });
...
1,2026-01-08T10:32:50.222Z,10000,CFOP,DEV,TEST,U,-1,Unknown`;
    ...
    test('Acubemy before cutoff remaps AUF by cross face, on/after cutoff does not', () => {
        const csv = `solve_id,date,total_time,analysis_type,cross_face,solution,move_times,cross_moves,f2l_pair1_moves
100,2025-10-21T12:00:00.000Z,5000,CFOP,U,R D D R,0 100 300 500,R,D D R
101,2025-10-22T00:00:00.000Z,5000,CFOP,U,R D D R,0 100 300 500,R,D D R`;
        const solves: Solve[] = parseCsv(csv, ',');
        expect(solves.length).toBe(2);

        const beforeCutoff = solves[0];
        const onCutoff = solves[1];
        expect(beforeCutoff.rawSourceId).toBe('100');
        expect(onCutoff.rawSourceId).toBe('101');

        const beforeF2L1 = beforeCutoff.steps[1];
        const onCutoffF2L1 = onCutoff.steps[1];
        expect(beforeF2L1.name).toBe('F2L Slot 1');
        expect(onCutoffF2L1.name).toBe('F2L Slot 1');

        // Cross face U => D-based AUF before cutoff, so leading D moves become pre-AUF.
        expect(beforeF2L1.preAufTime).toBeCloseTo(0.4, 5);
        // On/after cutoff falls back to standard U-based AUF, so leading D is not AUF.
        expect(onCutoffF2L1.preAufTime).toBeCloseTo(0, 5);
    });
    ...
    test('computeStepSegments supports custom AUF set', () => {
        const moveTimings = [{ move: "D", timestamp: 0 }, { move: "D2", timestamp: 500 }];
        const seg = computeStepSegments(moveTimings, null, StepName.PLL, aufMovesForFace('D'));
        expect(seg.preAuf).toBe(0);
        expect(seg.coreExecution).toBe(0);
        expect(seg.postAuf).toBeCloseTo(0.5, 5);
    });
